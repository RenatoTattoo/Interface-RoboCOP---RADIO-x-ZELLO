...
void loop()
{
  ...
  if ((millis() - TimerLOOP) > 20)
  {
    ...
    if (FalarMensagens) 
    {
      if (((M == 15) | (M == 45)) & (!FalouMensagem) & (EstadoZello == "On-line") & (EstadoRadio == "Stand by"))
      {
        LigarPTTradio();
        TOCAR(Vinheta);
        DesligarPTTradio();
      ...
      if ((!FalouMensagem) & ((M == 14) | (M == 44))) FalouHORA = false;
    }
...
void ESTADOzello()
{
  ...
  if ((MediaLDRzello[NAZ-1] > (MediaLDRzello[0] - Porcentagem)) & (MediaLDRzello[NAZ-1] < (MediaLDRzello[0] + Porcentagem)))
  {
   ...
   // {
      ...
      if ((LeituraZELLOatual >  500)  & (LeituraZELLOatual <  LIz))  EstadoZello = "Off-line";
      if  (LeituraZELLOatual <  499)                                 EstadoZello = "Tela Apagada";
   // } 
  }
...
void BootApertado()
{
  ...
  if (TipoMOSTRADOR > 3) TipoMOSTRADOR = 1;
  TimerLCD = millis();
}
