...
long MenuSETUP()
{
  while (1)
  {
    ...
    if (opcao == 6) display.println(F(">SAIR"));
    ...
    if (!digitalRead(BotaoBOOT))
    {
      ...
      while (!digitalRead(BotaoBOOT))
      {
        if ((millis() - Crono) > 2000) 
        {
          ...
          delay(1000);
          digitalWrite (LED, 0);
          if (opcao == 1) ADDPTTZello();
          if (opcao == 2) PosicionarLDRZello(); 
          if (opcao == 3) CalibrarLDRZello(); 
          if (opcao == 4) AjusteVOLUMEvoz(); 
          if (opcao == 5) WiFiconfig();         
          if (opcao == 6) ESP.restart();
          opcao--;
        }
      }
      opcao++;
