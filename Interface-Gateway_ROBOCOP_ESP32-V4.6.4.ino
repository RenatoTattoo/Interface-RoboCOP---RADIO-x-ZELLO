...
// relação MENSAGEM  com  o  NÚMERO DO ARQUIVO no cartão SD-------------------------------------------------------------------------------
 ...
 #define RogerBEEPweb     26               //Arquivo 25~31 correspondem aos áudios de OPERACIONAIS
...
void setup()
{
 ...
 // INICIANDO DISPLAY ---------------------------------------------------------------------------------------------------------------------
  ...
  display.println();
  display.println(F("---------------------"));
  display.println(F("  Para CONFIGURACAO"));
 ...
 // MODO CONFIGURAÇÃO ---------------------------------------------------------------------------------------------------------------------
  if (INICIO == 500) 
  {
    ...
    display.setTextSize(1);
    display.setCursor (0, 0);
    display.print(F("  VOLUME VOZ = "));
    display.println(Volume); 
    display.println(F("---------------------")); 
    display.println(F(" ")); 
    display.println(F("    PARA ALTERAR "));
    display.println(F("  Pressione o BOOT"));
    display.display();
    delay (INICIO);
    Crono      = millis();
    TimerPisca = millis();
   ...
   // CONFIGURAÇÃO BOTÃO PTT DO ZELLO -----------------------------------------------------------------------------------------------------
    digitalWrite (LED, 1);
    display.setCursor (0, 0);
    display.setTextColor(SSD1306_WHITE);
    display.println(F("ADD BOTAO PTT ZELLO?"));
    display.println(F("---------------------"));
    display.println(F(" "));
    display.println(F("  Pressione o BOOT"));
    display.display();
   ...
   // POSICIONAMENTO E CONFIGURAÇÃO LDR ZELLO ----------------------------------------------------------------------------------------
   ...
   // CONFIGURAÇÃO DO LDR ---------------------------------------------------------------------------------------------------------- 
    ...
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.println(F("CONFIGURAR LDR ZELLO?"));
    display.println(F("---------------------"));
    display.println();
    display.println(F(" Pressione o BOOT"));
    display.display();
...
void loop()
{
  ...
  {  
    ...
    if ((WIFIok) & (M == 30) & (!FalouAsHORAS) & (HC=="+"))
    {
      PTTradioON();
      Crono = millis();
      while ((millis() - Crono) < 200) Display();
    ...
    if ((((M == 15) | (M == 45)) & (S == 0) & (Narquivos >= PrimeiraVinheta)) | (BOOTacionado))
    {
      PTTradioON();
      Crono = millis();
      while ((millis() - Crono) < 200) Display();
...
long ESTADOzello() // TelaAPAGOU TA - Offline OF - RX - TX - Esperando ON - TelaInicial TI
{
  ...
  RADIO = digitalRead(SinalLDRradio);  
...
{
  display.println("Configuração salva");
  display.print(F("IP: "));
  display.println(WiFi.softAPIP()); //imprime o IP do AP
  display.display();
  delay(2000);  
