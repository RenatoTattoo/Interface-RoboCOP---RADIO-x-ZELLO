...
long ESTADOzello() // TelaAPAGOU TA - Offline OF - RX - TX - Esperando ON - TelaInicial TI
{
  ...
  if (MediaLDR[1] == MediaLDR[NCc-1])
  {
   ...
   {  
      if ((VON - ((VON / 100) * 5)) < LeituraLDRzello < (VON + ((VON / 100) * 5))) VON = mmON();     //escluindo discrepancias acima de 5% de diferença da amostragem anterior
      CronoON = millis(); 
   }
   if (statusZELLO != "RX") CronoRX = millis();     
   if ((statusZELLO == "RX") & ((millis() - CronoRX) > 25000))
   {  
      if ((VRX - ((VRX / 100) * 3)) < LeituraLDRzello < (VRX + ((VRX / 100) * 3))) VRX = mmRX();     //escluindo discrepancias acima de 3% de diferença da amostragem anterior
      CronoRX = millis() - 12000; 
   } 
   if (statusZELLO != "TX") CronoTX = millis(); 
   if ((statusZELLO == "TX") & ((millis() - CronoTX) > 25000))
   {  
      if ((VTX - ((VTX / 100) * 3)) < LeituraLDRzello < (VTX + ((VTX / 100) * 3))) VTX = mmTX();     //escluindo discrepancias acima de 3% de diferença da amostragem anterior
...
long Display()
{
  ...
  {    
    ...
    {
      TimerDisplay = (millis() - TempoDisplay + 4000);
      TipoMOSTRADOR = 1;
    }
    if ((millis() - TimerDisplay < TempoDisplay) | (statusZELLO !="ON"))   
    {   
...
long MenuSETUPinicio()
{
  ...
  while (Sair)
  {
    ...
    display.display();
    if ((millis() - Timer) > 10000) Sair = false;
  }
  opcao = 0;
  TipoMOSTRADOR = 0;
}
...
long CalibrarLDRZello()
{
  ...
  EEPROM.commit();
  for(int p=0; p<NCo; p++) MediaON[p] = LDRon;
  for(int p=0; p<NCo; p++) MediaRX[p] = LDRrx;
  for(int p=0; p<NCo; p++) MediaTX[p] = LDRtx;
  digitalWrite (LED, 1);
  delay (500);
  digitalWrite (LED, 0);
}
...
long AjusteVOLUMEvoz()
{
  unsigned long CronoSETUP = millis();
  digitalWrite (LED, 0);
  display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);
  display.setTextSize(1);
  ...
  while ((millis() - CronoSETUP) < 5000)
  {
    ...
    if (((millis() - CronoSETUP) > 1000) & ((millis() - CronoSETUP) < 1100))
    {
      PTTradioON();
      LOCUTORA.play (LinkONLINE);
