...
// Outras Definições ---------------------------------------------------------------------------------------------------------------------------------
 ...
 #define NAZ              100             //Número de amostragens para a média móvel da leitura do LDR do ZELLO
 #define NAAz             20              //Número de amostragens para a média móvel das correções de auto ajuste Zello
 #define NAR              500             //Número de amostragens para a média móvel da leitura do LDR do RADIO
...
void setup() 
{
  ...
  display.display();
  display.clearDisplay();
  digitalWrite (FANradio, Desligado);
  delay(1000);
}
...
long mmONRXz()
{
  ...
  if (EstadoZello == "Recebendo") MediaONz[0] = (LeituraZELLOatual + DifONRXz);
  long contador = 0;
...
void ValoresLimites()
{
  ...
  L1r = LSr / 5;                                                  // calculando limite de ativação entre stand-by e RX/TX. 
}

void ESTADOzello()
{
  ValoresLimites();
  int Porcentagem = ((NAZ / 100) * 90);
  if ((MediaLDRzello[NAZ-1] > (MediaLDRzello[0] - Porcentagem)) && (MediaLDRzello[NAZ-1] < (MediaLDRzello[0] + Porcentagem)))
  {
    if ((millis() - TimerDeEntrada) > 20)                         //Aguardar a leitura se estabilizar por este tempo
    {
...
void TEMPERATURAradio()
{
 ...
 //#define Ro               10000           //Resistência do termistor à 25°C
  ...
  {
    case 1:
      Periodo = 1000;              // Gráfico de temperatura com 2 minutos de registro
    break; 
    case 2:
      Periodo = 56250;             // Gráfico de temperatura com 2 horas de registro
    break;
    case 3: 
      Periodo = 168750;            // Gráfico de temperatura com 6 horas de registro
    break;
    case 4:
      Periodo = 337500;            // Gráfico de temperatura com 12 horas de registro
    break;
    case 5:
      Periodo = 675000;            // Gráfico de temperatura com 24 horas de registro
    break;
  }
